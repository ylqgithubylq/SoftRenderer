...
namespace X
{
	...
	public:
		...
		struct InputEvent
		{
			...
			*/
			s32 data;
			Point<s32, 2> pointerPosition;
			f64 currentTime;

			InputEvent(s32 mappedSemantic, s32 data, Point<s32, 2> pointerPosition, f64 currentTime)
				: mappedSemantic(mappedSemantic), data(data), pointerPosition(pointerPosition), currentTime(currentTime)
			{
			}
		};
	...
	public:
		...
		bool AddInputHandler(std::shared_ptr<InputHandler> inputHandler);

		bool RemoveInputHandler(std::shared_ptr<InputHandler> const& inputHandler);

		bool GetKeyState(InputSemantic semantic) const
		{
			return semanticStates_[static_cast<u32>(semantic)];
		}

		Point<s32, 2> GetPointerPosition() const
		{
			return pointerPosition_;
		}

		Point<s32, 2> GetPreviousPointerPosition() const
		{
		...
		*/
		void ExecuteAllQueuedActions(f64 currentTime);

		void InjectKeyDown(InputSemantic semantic);
		void InjectKeyUp(InputSemantic semantic);
		void InjectMouseDown(InputSemantic semantic, s32 x, s32 y);
		void InjectMouseUp(InputSemantic semantic, s32 x, s32 y);
		void InjectMouseWheel(InputSemantic semantic, s32 x, s32 y, s32 delta);
		void InjectMouseMove(InputSemantic semantic, s32 x, s32 y);
	private:
		void DispatchInputEvent(InputSemantic semantic, s32 data);

	private:
		Context& context_;

		std::array<bool, static_cast<u32>(InputSemantic::InputSemanticCount)> semanticStates_;
		Point<s32, 2> previousPointerPosition_;
		Point<s32, 2> pointerPosition_;

		std::unordered_set<std::shared_ptr<InputHandler>> inputHandlers_;

		std::queue<std::function<void()>> actionQueue_;
	};
}
